...
public class ReservasView extends JFrame {
	...
	private Integer valorAcobrarPorDia;
...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
		...
		txtValor.setBounds(180, 328, 200, 33);
	...
	// manejador de vento siquiente
	public void manejadorBntSiguiente(){
		Date dateE = ReservasView.txtFechaE.getDate();
		Date dateS = ReservasView.txtFechaS.getDate();
		...
		Integer numeroDeDiasReservados = Math.round((dateS.getTime() - dateE.getTime()) / 86400000);

		if (numeroDeDiasReservados<=0) {
			JOptionPane.showMessageDialog(null, "Debes reservar minimo un dia, por favor revise sus fechas.");
			return;
		}
		Integer valor = numeroDeDiasReservados*valorAcobrarPorDia;
		Reserva reserva= new Reserva(simpleDateFormat.format(dateE),simpleDateFormat.format(dateS),valor,txtFormaPago.getSelectedItem().toString());

		if (reservaController.reservar(reserva)){
			JOptionPane.showMessageDialog(null, "Reserva generada exitosamente!!");
	...
	public void manejadorDeCambio(){
		...
		txtValor.setText((numeroDeDiasReservados*valorAcobrarPorDia) + "$");
	}
}
