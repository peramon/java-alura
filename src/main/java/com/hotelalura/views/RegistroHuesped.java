...
public class RegistroHuesped extends JFrame {
	...
	private void headerMouseDragged(java.awt.event.MouseEvent evt) {
		...
		int y = evt.getYOnScreen();this.setLocation(x - xMouse, y - yMouse);
	}
	//btn regresar
	public void manejadorRegresar(){
		int selecion = JOptionPane.showConfirmDialog(
				null,
				"Si regresas se eliminara tu avance." +
						"\nEstas de acuerdo?",
				"Advertencia!",
		...
		if (selecion == JOptionPane.YES_OPTION) {
			...
			reservaController.eliminarUltimoReserva(reserva.getId());
			dispose();
		}
	}
	//btn guardar
	public void manejadorGuardar(){
		...
		if(nombre.isBlank()|| apellido.isBlank() || telefono.isBlank()||nacionalidad.isBlank()||date == null){
			JOptionPane.showMessageDialog(null, "Debes llenar todos los campos.");
			return;
		}
		if(!comprobarSiPuedeSerUnNumeroDeTelefono(telefono)){
			JOptionPane.showMessageDialog(null, "Debes llenar el campo\nteléfono solo con números");
			return;
		}
		Huesped huesped = new Huesped(nombre,apellido,simpleDateFormat.format(date),nacionalidad,telefono,reserva.getId());
		Integer direrencia =  Math.round((dateHoy.getTime() - date.getTime()) / 31557600000L);
		if(direrencia<18){
			JOptionPane.showMessageDialog(null, "Debes tener 18 o mas años");
			return;
		}
		if(huespedController.guardarHuesped(huesped)){
	...
	public boolean comprobarSiPuedeSerUnNumeroDeTelefono(String s) {
		if(s.length()<8){
			return false;
		}
